...
function buildProgressBar(current, max, colorEmoji, emptyEmoji = 'Γ¼¢', length = 10) {
...
// ΓöÇΓöÇΓöÇ Playable Characters ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
const PLAYABLE_CHARACTERS = [
    {
        ...
        abilities: [
            { 
                ...
                desc: '≡ƒÆÑ Fires a powerful energy wave dealing 18-28 damage.', 
                execute: (p, o) => {
                    ...
                    return `≡ƒÆÑ **Goku** fired a massive **Kamehameha** at **${o.name}** dealing **${dmg}** damage!`;
                }
            },
            { 
                name: 'Super Saiyan', 
                ...
                desc: 'ΓÜí Powers up! Gains +30 Energy, heals 15 HP, and boosts next attack by 40%.', 
                execute: (p, o) => {
                    p.ssjBuff = true;
                    p.energy = Math.min(100, p.energy + 30);
                    p.hp = Math.min(p.maxHp, p.hp + 15);
                    return `ΓÜí **Goku** powered up into **Super Saiyan**! Heals 15 HP, gains +30 Energy, and boosts next attack's damage by 40%!`;
                }
            },
            { 
                ...
                action: 'run', 
                style: ButtonStyle.Primary,
                desc: '≡ƒîÇ Evades the next attack and counters for 10 damage.', 
                execute: (p, o) => {
                    p.dodge = true;
                    p.dodgeCounter = 10;
                    return `≡ƒîÇ **Goku** vanishes using **Instant Transmission**, preparing to evade the next attack and counter-attack!`;
                }
            },
            { 
                name: 'Spirit Bomb', 
                ...
                desc: '≡ƒîƒ Gathers planetary energy to hurl a colossal Spirit Bomb dealing 40-60 damage.', 
                execute: (p, o) => {
                    let dmg = randRange(40, 60);
                    if (p.ssjBuff) { dmg = Math.floor(dmg * 1.4); p.ssjBuff = false; }
                    if (o.shield) { dmg = Math.floor(dmg * (1 - o.shieldRate)); }
                    o.hp = Math.max(0, o.hp - dmg);
                    return `≡ƒîƒ **Goku** gathered the energy of the universe and hurled a colossal **Spirit Bomb** at **${o.name}** for **${dmg}** massive damage!`;
                }
            }
        ]
    },
    {
        ...
        abilities: [
            { 
                ...
                action: 'yeet', 
                style: ButtonStyle.Primary,
                desc: '≡ƒö┤ Deals 16-24 repelling damage.', 
                execute: (p, o) => {
                    let dmg = randRange(16, 24);
                    if (o.shield) { dmg = Math.floor(dmg * (1 - o.shieldRate)); }
                    o.hp = Math.max(0, o.hp - dmg);
                    return `≡ƒö┤ **Gojo** released Curse Technique Reversal **Red** dealing **${dmg}** damage to **${o.name}**!`;
                }
            },
            { 
                ...
                action: 'smile', 
                style: ButtonStyle.Success,
                desc: '≡ƒ¢í∩╕Å Deploys Infinity. Reduces incoming damage by 80% for 2 turns and heals 10 HP.', 
                execute: (p, o) => {
                    p.shield = true;
                    p.shieldTurns = 2;
                    p.shieldRate = 0.8;
                    p.hp = Math.min(p.maxHp, p.hp + 10);
                    return `≡ƒ¢í∩╕Å **Gojo** deployed his **Infinity** shield! Incoming damage reduced by 80% for 2 turns. Heals 10 HP.`;
                }
            },
            { 
                ...
                action: 'slap', 
                style: ButtonStyle.Primary,
                desc: '≡ƒö╡ Deals 10 damage and stuns the opponent for 1 turn.', 
                execute: (p, o) => {
                    let dmg = 10;
                    if (o.shield) { dmg = Math.floor(dmg * (1 - o.shieldRate)); }
                    o.hp = Math.max(0, o.hp - dmg);
                    o.stunned = true;
                    return `≡ƒö╡ **Gojo** used Curse Technique Lapse **Blue** to pull **${o.name}** in, dealing **${dmg}** damage and **stunning** them for 1 turn!`;
                }
            },
            { 
                name: 'Unlimited Void', 
                ...
                action: 'stare', 
                style: ButtonStyle.Danger,
                desc: '≡ƒîî Expands domain. Deals 35-45 absolute damage (ignores shield) and stuns for 1 turn.', 
                execute: (p, o) => {
                    let dmg = randRange(35, 45); // ignores shields
                    o.hp = Math.max(0, o.hp - dmg);
                    o.stunned = true;
                    return `≡ƒîî **Gojo** expanded his domain: **Unlimited Void**! Brain-overloading **${o.name}**, dealing **${dmg}** absolute damage and **stunning** them for 1 turn!`;
                }
            }
        ]
    },
    {
        ...
        abilities: [
            { 
                ...
                action: 'punch', 
                style: ButtonStyle.Primary,
                desc: 'Γ£è Standard punch dealing 12-16 damage. Gains +20 Energy.', 
                execute: (p, o) => {
                    let dmg = randRange(12, 16);
                    if (o.shield) { dmg = Math.floor(dmg * (1 - o.shieldRate)); }
                    o.hp = Math.max(0, o.hp - dmg);
                    p.energy = Math.min(100, p.energy + 20);
                    return `Γ£è **Saitama** threw a **Normal Punch** at **${o.name}** dealing **${dmg}** damage and gaining +20 Energy!`;
                }
            },
            { 
                name: 'Serious Dodge', 
                ...
                action: 'run', 
                style: ButtonStyle.Success,
                desc: '≡ƒÆ¿ Evades all attacks next turn, gains +30 Energy, and heals 10 HP.', 
                execute: (p, o) => {
                    p.dodge = true;
                    p.dodgeCounter = 0;
                    p.energy = Math.min(100, p.energy + 30);
                    p.hp = Math.min(p.maxHp, p.hp + 10);
                    return `≡ƒÆ¿ **Saitama** executed a **Serious Dodge** side-step! Evading all attacks next turn, gaining +30 Energy, and healing 10 HP.`;
                }
            },
            { 
                ...
                action: 'punch', 
                style: ButtonStyle.Primary,
                desc: '≡ƒæè Rapid punches dealing 16-30 damage.', 
                execute: (p, o) => {
                    ...
                    return `≡ƒæè **Saitama** threw **Consecutive Normal Punches** at **${o.name}** landing **${hits}** hits for **${total}** total damage!`;
                }
            },
            { 
                name: 'Serious Punch', 
                ...
                action: 'yeet', 
                style: ButtonStyle.Danger,
                desc: '≡ƒÆÑ Serious Series: Serious Punch! Deals 60-80 apocalyptic damage.', 
                execute: (p, o) => {
                    let dmg = randRange(60, 80);
                    if (o.shield) { dmg = Math.floor(dmg * (1 - o.shieldRate)); }
                    o.hp = Math.max(0, o.hp - dmg);
                    return `≡ƒÆÑ **Saitama** unleashed a **Serious Punch**! The shockwave blasts **${o.name}** for **${dmg}** colossal damage, parting the clouds!`;
                }
            }
        ]
    },
    {
        ...
        abilities: [
            { 
                ...
                action: 'punch', 
                style: ButtonStyle.Primary,
                desc: '≡ƒîÇ Swirling sphere of chakra dealing 16-26 damage.', 
                execute: (p, o) => {
                    let dmg = randRange(16, 26);
                    if (p.kuramaBuff) { dmg = Math.floor(dmg * 1.3); }
                    if (o.shield) { dmg = Math.floor(dmg * (1 - o.shieldRate)); }
                    o.hp = Math.max(0, o.hp - dmg);
                    return `≡ƒîÇ **Naruto** charges forward with a **Rasengan**, smashing **${o.name}** for **${dmg}** damage!`;
                }
            },
            { 
                ...
                action: 'dance', 
                style: ButtonStyle.Primary,
                desc: '≡ƒæÑ Summons clones. Evades the next attack completely and counters for 10 damage.', 
                execute: (p, o) => {
                    p.dodge = true;
                    p.dodgeCounter = 10;
                    return `≡ƒæÑ **Naruto** creates a squad of **Shadow Clones**! They prepare to shield the real Naruto and counter-attack.`;
                }
            },
            { 
                ...
                action: 'angry', 
                style: ButtonStyle.Success,
                desc: '≡ƒªè Heals 20 HP, gains +40 Energy, and boosts all damage by 30% for 3 turns.', 
                execute: (p, o) => {
                    p.kuramaBuff = true;
                    p.kuramaBuffTurns = 3;
                    p.energy = Math.min(100, p.energy + 40);
                    p.hp = Math.min(p.maxHp, p.hp + 20);
                    return `≡ƒªè **Naruto** enters **Kurama Sage Mode**! Heals 20 HP, gains +40 Energy, and gains a 30% damage boost for 3 turns!`;
                }
            },
            { 
                name: 'Rasenshuriken', 
                ...
                action: 'yeet', 
                style: ButtonStyle.Danger,
                desc: '≡ƒî¬∩╕Å Throws a wind-shuriken of dense Tailed Beast chakra dealing 38-50 damage.', 
                execute: (p, o) => {
                    let dmg = randRange(38, 50);
                    if (p.kuramaBuff) { dmg = Math.floor(dmg * 1.3); }
                    if (o.shield) { dmg = Math.floor(dmg * (1 - o.shieldRate)); }
                    o.hp = Math.max(0, o.hp - dmg);
                    return `≡ƒî¬∩╕Å **Naruto** hurls a **Tailed Beast Rasenshuriken**! The exploding wind vortex shreds **${o.name}** for **${dmg}** massive damage!`;
                }
            }
        ]
    },
    {
        ...
        abilities: [
            { 
                ...
                action: 'punch', 
                style: ButtonStyle.Primary,
                desc: '≡ƒÑè Stretches his arm for a quick punch dealing 14-18 damage. Gains +25 Energy.', 
                execute: (p, o) => {
                    let dmg = randRange(14, 18);
                    if (o.shield) { dmg = Math.floor(dmg * (1 - o.shieldRate)); }
                    o.hp = Math.max(0, o.hp - dmg);
                    p.energy = Math.min(100, p.energy + 25);
                    return `≡ƒÑè **Luffy** stretches his arm and lands a **Gum-Gum Pistol** on **${o.name}** for **${dmg}** damage, gaining +25 Energy!`;
                }
            },
            { 
                name: 'Gear 2nd Speed', 
                ...
                action: 'run', 
                style: ButtonStyle.Primary,
                desc: 'ΓÜí Deals 10 damage and grants an immediate extra turn!', 
                execute: (p, o) => {
                    let dmg = 10;
                    if (o.shield) { dmg = Math.floor(dmg * (1 - o.shieldRate)); }
                    o.hp = Math.max(0, o.hp - dmg);
                    p.extraTurn = true;
                    return `ΓÜí **Luffy** activates **Gear 2nd**! Strikes **${o.name}** for **${dmg}** damage and speeds up, gaining an immediate extra turn!`;
                }
            },
            { 
                ...
                action: 'stare', 
                style: ButtonStyle.Success,
                desc: '≡ƒ¢í∩╕Å Reduces damage by 60% and reflects 30% of incoming damage next turn.', 
                execute: (p, o) => {
                    p.shield = true;
                    p.shieldTurns = 1;
                    p.shieldRate = 0.6;
                    p.reflect = true;
                    return `≡ƒ¢í∩╕Å **Luffy** hardens with Armament Haki! Reduces damage taken by 60% and reflects 30% of incoming damage!`;
                }
            },
            { 
                name: 'Bajrang Gun', 
                ...
                action: 'yeet', 
                style: ButtonStyle.Danger,
                desc: 'ΓÿÇ∩╕Å Sun God Nika! Deals 35-55 damage and stuns the target for 1 turn.', 
                execute: (p, o) => {
                    let dmg = randRange(35, 55);
                    o.hp = Math.max(0, o.hp - dmg);
                    o.stunned = true;
                    return `ΓÿÇ∩╕Å **Luffy** laughs joyfully in **Gear 5th** and crushes **${o.name}** with a giant **Bajrang Gun** for **${dmg}** damage, **stunning** them!`;
                }
            }
        ]
    },
    {
        ...
        abilities: [
            { 
                ...
                action: 'shoot', 
                style: ButtonStyle.Primary,
                desc: '≡ƒîè Sharp water blades dealing 15-22 damage.', 
                execute: (p, o) => {
                    let dmg = randRange(15, 22);
                    if (o.shield) { dmg = Math.floor(dmg * (1 - o.shieldRate)); }
                    o.hp = Math.max(0, o.hp - dmg);
                    return `≡ƒîè **Rimuru** fires sharp, slicing **Water Blades** at **${o.name}** dealing **${dmg}** damage!`;
                }
            },
            { 
                name: 'Gluttony', 
                ...
                action: 'bite', 
                style: ButtonStyle.Primary,
                desc: '≡ƒÿê Devours enemy energy. Deals 15-20 damage and heals Rimuru for the same amount.', 
                execute: (p, o) => {
                    let dmg = randRange(15, 20);
                    if (o.shield) { dmg = Math.floor(dmg * (1 - o.shieldRate)); }
                    o.hp = Math.max(0, o.hp - dmg);
                    p.hp = Math.min(p.maxHp, p.hp + dmg);
                    return `≡ƒÿê **Rimuru** activates **Gluttony**, devouring **${o.name}**'s lifeforce. Deals **${dmg}** damage and heals Rimuru for that amount!`;
                }
            },
            { 
                ...
                action: 'smile', 
                style: ButtonStyle.Success,
                desc: '≡ƒºá Gains +35 Energy, heals 10 HP, and next attack deals +30% damage.', 
                execute: (p, o) => {
                    p.sageBuff = true;
                    p.energy = Math.min(100, p.energy + 35);
                    p.hp = Math.min(p.maxHp, p.hp + 10);
                    return `≡ƒºá **Rimuru** consults **Great Sage**! Analyzes enemy weakness, gains +35 Energy, heals 10 HP, and boosts next attack by 30%!`;
                }
            },
            { 
                name: 'Megiddo', 
                ...
                action: 'shoot', 
                style: ButtonStyle.Danger,
                desc: 'ΓÿÇ∩╕Å Solar rays deal 35-45 damage and burn for 6 damage/turn for 2 turns.', 
                execute: (p, o) => {
                    ...
                    o.burnTurns = 2;
                    o.burnDmg = 6;
                    return `ΓÿÇ∩╕Å **Rimuru** executes **Megiddo**! Countless solar rays pierce **${o.name}** dealing **${dmg}** damage and leaving them **burned**!`;
                }
            }
        ]
    }
...
// ΓöÇΓöÇΓöÇ Boss Opponents ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
const BOSSES = [
    {
        name: 'Sukuna',
        emoji: '≡ƒÆÇ',
        ...
        abilities: [
            { 
                ...
                execute: (p, o) => {
                    ...
                    if (o.shield) dmg = Math.floor(dmg * (1 - o.shieldRate));
                    o.hp = Math.max(0, o.hp - dmg);
                    return `ΓÜö∩╕Å **Sukuna** casts **Dismantle** dealing **${dmg}** slashing damage to **${o.name}**!`;
                }
            },
            { 
                name: 'Cleave', 
                ...
                execute: (p, o) => {
                    ...
                    if (o.shield) dmg = Math.floor(dmg * (1 - o.shieldRate));
                    o.hp = Math.max(0, o.hp - dmg);
                    return `ΓÜö∩╕Å **Sukuna** casts **Cleave** on **${o.name}** dealing **${dmg}** high-slashing damage!`;
                }
            },
            { 
                name: 'Flame Arrow', 
                ...
                execute: (p, o) => {
                    ...
                    if (o.shield) dmg = Math.floor(dmg * (1 - o.shieldRate));
                    o.hp = Math.max(0, o.hp - dmg);
                    return `≡ƒöÑ **Sukuna** chants and fires his **Flame Arrow (Fuga)** hitting **${o.name}** for **${dmg}** fire damage!`;
                }
            },
            { 
                ...
                execute: (p, o) => {
                    let dmg = randRange(35, 45); // ignores shields
                    o.hp = Math.max(0, o.hp - dmg);
                    return `Γ¢⌐∩╕Å **Sukuna** expands his domain: **Malevolent Shrine**! A storm of cuts shreds **${o.name}** for **${dmg}** absolute damage!`;
                }
            }
        ]
    },
        ...
        abilities: [
            { 
                ...
                execute: (p, o) => {
                    ...
                    if (o.shield) dmg = Math.floor(dmg * (1 - o.shieldRate));
                    o.hp = Math.max(0, o.hp - dmg);
                    return `ΓÜö∩╕Å **Madara** swings his **Susanoo Blade** dealing **${dmg}** slash damage to **${o.name}**!`;
                }
            },
            { 
                ...
                execute: (p, o) => {
                    ...
                    if (o.shield) dmg = Math.floor(dmg * (1 - o.shieldRate));
                    o.hp = Math.max(0, o.hp - dmg);
                    return `≡ƒöÑ **Madara** spews a sea of fire: **Majestic Destroyer Flame**, burning **${o.name}** for **${dmg}** fire damage!`;
                }
            },
            { 
                ...
                execute: (p, o) => {
                    ...
                    o.hp = Math.max(0, o.hp - dmg);
                    o.stunned = true;
                    return `≡ƒ¬╡ **Madara** uses **Deep Forest Emergence**, trapping **${o.name}** for **${dmg}** damage and **stunning** them for 1 turn!`;
                }
            },
            { 
                ...
                execute: (p, o) => {
                    ...
                    if (o.shield) dmg = Math.floor(dmg * (1 - o.shieldRate));
                    o.hp = Math.max(0, o.hp - dmg);
                    return `Γÿä∩╕Å **Madara** summons a colossal meteor: **Shattered Heaven**! The sky falls upon **${o.name}** dealing **${dmg}** devastating damage!`;
                }
            }
        ]
    },
        ...
        abilities: [
            { 
                ...
                execute: (p, o) => {
                    ...
                    if (o.shield) dmg = Math.floor(dmg * (1 - o.shieldRate));
                    o.hp = Math.max(0, o.hp - dmg);
                    return `≡ƒÑè **Kaido** swings his spiked iron club dealing **${dmg}** blunt damage to **${o.name}**!`;
                }
            },
            { 
                name: 'Bolo Breath', 
                ...
                execute: (p, o) => {
                    ...
                    if (o.shield) dmg = Math.floor(dmg * (1 - o.shieldRate));
                    o.hp = Math.max(0, o.hp - dmg);
                    return `≡ƒöÑ **Kaido** releases **Bolo Breath** from his dragon mouth, scorching **${o.name}** for **${dmg}** fire damage!`;
                }
            },
            { 
                ...
                execute: (p, o) => {
                    ...
                    return `≡ƒÉ▓ **Kaido** hardens his dragon scales! Heals 12 HP and reduces incoming damage by 50% for 2 turns.`;
                }
            },
            { 
                name: 'Thunder Bagua', 
                ...
                execute: (p, o) => {
                    ...
                    if (o.shield) dmg = Math.floor(dmg * (1 - o.shieldRate));
                    o.hp = Math.max(0, o.hp - dmg);
                    return `ΓÜí **Kaido** blitzes forward at god speed, unleashing **Thunder Bagua** and smashing **${o.name}** for **${dmg}** lightning damage!`;
                }
            }
        ]
    },
        ...
        abilities: [
            { 
                ...
                execute: (p, o) => {
                    ...
                    if (o.shield) dmg = Math.floor(dmg * (1 - o.shieldRate));
                    o.hp = Math.max(0, o.hp - dmg);
                    return `≡ƒæë **Frieza** fires a piercing **Death Beam** dealing **${dmg}** damage to **${o.name}**!`;
                }
            },
            { 
                ...
                execute: (p, o) => {
                    ...
                    if (o.shield) dmg = Math.floor(dmg * (1 - o.shieldRate));
                    o.hp = Math.max(0, o.hp - dmg);
                    return `≡ƒîÇ **Frieza** lifts **${o.name}** telekinetically and detonates them with a **Death Psycho Bomb** for **${dmg}** damage!`;
                }
            },
            { 
                ...
                execute: (p, o) => {
                    p.hp = Math.min(p.maxHp, p.hp + 15);
                    p.goldBuff = true;
                    return `Γ£¿ **Frieza** transforms into **Golden Frieza**! Heals 15 HP and his next attack deals 40% more damage!`;
                }
            },
            { 
                name: 'Supernova', 
                ...
                execute: (p, o) => {
                    ...
                    if (o.shield) dmg = Math.floor(dmg * (1 - o.shieldRate));
                    o.hp = Math.max(0, o.hp - dmg);
                    return `Γÿä∩╕Å **Frieza** drops a planet-destroying **Supernova** on **${o.name}** for **${dmg}** absolute damage!`;
                }
            }
        ]
    }
...
module.exports = {
    ...
    async executePrefix(message, args) {
        ...
        if (isNaN(bet) || bet < 0) {
            return message.reply('Γ¥î Please enter a valid non-negative integer for your bet. (Example: `-ab 500` or `-ab 0` for fun!)');
        }

        if (bet > 0 && bet < 200) {
            return message.reply('Γ¥î The minimum bet to gamble is **200** Baubles. Set your bet to **0** to play for free!');
        }
...
async function runAnimeBattle({ context, userId, user, bet, isSlash }) {
    try {
        ...
        if (bet > 0 && baubleData.baubles < bet) {
            const errorMsg = `Γ¥î You only have **${baubleData.baubles}** Baubles. You cannot bet **${bet}** Baubles.`;
            return isSlash 
        ...
        const selectEmbed = new EmbedBuilder()
            .setColor(0x3498db)
            .setTitle('≡ƒÄ« CHOOSE YOUR ANIME FIGHTER')
        ...
        try {
            ...
            const timeoutEmbed = new EmbedBuilder()
                .setColor(0x747f8d)
                .setTitle('ΓÅ░ SELECTION TIMED OUT')
        ...
        if (bet > 0 && (!baubleData || baubleData.baubles < bet)) {
            const errEmbed = new EmbedBuilder()
                .setColor(0xff0000)
                .setTitle('Γ¥î TRANSACTION FAILED')
        ...
        let turn = 'player';
        let turnCount = 1;
        let lastActionLog = `ΓÜö∩╕Å The arena vibrates! **${player.name}** faces off against the legendary **${boss.name}**!\nYour bet of **${bet.toLocaleString()} Baubles** has been locked.`;
        ...
        function buildBattleEmbed(footerText = `Anime: ${lastActionAnime} | Turn ${turnCount}`) {
            const pStatus = [];
            if (player.shield) pStatus.push('≡ƒ¢í∩╕Å Shielded');
            if (player.dodge) pStatus.push('≡ƒÆ¿ Evading');
            if (player.ssjBuff) pStatus.push('ΓÜí Super Saiyan');
            if (player.kuramaBuff) pStatus.push(`≡ƒªè Sage (${player.kuramaBuffTurns}t)`);
            if (player.sageBuff) pStatus.push('≡ƒºá Analyzed');
            if (player.stunned) pStatus.push('≡ƒÆ½ Stunned');
            if (player.burned) pStatus.push(`≡ƒöÑ Burned (${player.burnTurns}t)`);

            const bStatus = [];
            if (boss.shield) bStatus.push('≡ƒ¢í∩╕Å Shielded');
            if (boss.dodge) bStatus.push('≡ƒÆ¿ Evading');
            if (boss.goldBuff) bStatus.push('Γ£¿ Golden Form');
            if (boss.stunned) bStatus.push('≡ƒÆ½ Stunned');
            if (boss.burned) bStatus.push(`≡ƒöÑ Burned (${boss.burnTurns}t)`);
            ...
            return new EmbedBuilder()
                ...
                .setTitle(`ΓÜö∩╕Å ANIME SHOWDOWN ΓÇö Turn ${turnCount}`)
                .setDescription(`> ${lastActionLog}\n\n${turn === 'player' ? `≡ƒƒó **It is your turn!** Select an ability.` : `≡ƒö┤ **${boss.name}** is preparing their attack...`}`)
                .addFields(
                    {
                        name: `${chosenChar.emoji} ${player.name} (You)`,
                        value: `**HP:** ${buildProgressBar(player.hp, player.maxHp, '≡ƒƒ⌐')}${pStatusStr}\n**Energy:** ${buildProgressBar(player.energy, 100, '≡ƒƒª', 'Γ¼¢')}`,
                        inline: false
                    },
                    {
                        name: `${chosenBoss.emoji} ${boss.name} (Boss)`,
                        value: `**HP:** ${buildProgressBar(boss.hp, boss.maxHp, '≡ƒƒÑ')}${bStatusStr}\n**Energy:** ${buildProgressBar(boss.energy, 100, '≡ƒƒº', 'Γ¼¢')}`,
                        inline: false
                    }
                )
        ...
        while (!isGameOver) {
            ...
            if (turn === 'player' && player.burned) {
                ...
                lastActionLog = `≡ƒöÑ **${player.name}** takes **${player.burnDmg}** burn damage at the start of their turn!`;
                player.burnTurns--;
                ...
                lastActionLog = `≡ƒöÑ **${boss.name}** takes **${boss.burnDmg}** burn damage at the start of their turn!`;
                boss.burnTurns--;
            ...
            if (turn === 'player' && player.stunned) {
                ...
                lastActionLog = `≡ƒÆ½ **${player.name}** is stunned/confused and skips their turn!`;
                ...
                lastActionLog = `≡ƒÆ½ **${boss.name}** is stunned/confused and skips their turn!`;
                lastActionGif = 'https://media.giphy.com/media/v1.Y2lkPTc5MGI3NjExd2ptMXR6aGl6YTVicGRrcG13b2phd3plcnEydnE5eXUwcTFscWFqbyZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9Zw/3o7bu3XilJ5BOiSGic/giphy.gif';
                lastActionAnime = 'Unknown Anime';
            ...
            // ΓöÇΓöÇΓöÇ Player Turn ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
            if (turn === 'player') {
                ...
                if (boss.dodge && ability.cost > 0) {
                    boss.dodge = false;
                    let evadeText = `≡ƒÆ¿ **${boss.name}** completely evaded **${player.name}**'s **${ability.name}**!`;
                ...
                if (player.extraTurn) {
                    ...
                    lastActionLog = `ΓÜí **${player.name}** speed blitzes and gains an extra turn immediately!`;
                } else {
                    turn = 'boss';
                }
            ...
            // ΓöÇΓöÇΓöÇ Boss Turn ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
            else {
                boss.energy = Math.min(100, boss.energy + 20);
                ...
                if (player.dodge) {
                    player.dodge = false;
                    let evadeText = `≡ƒÆ¿ **${player.name}** completely evaded **${boss.name}**'s **${bossAbility.name}**!`;
                    ...
                    if (player.reflect && bossAbility.cost > 0) {
                        ...
                        logText += `\nΓÜí **${player.name}** reflected **${reflectedDmg}** damage back at **${boss.name}**!`;
                    }
        ...
        if (gameForfeit) {
            ...
            finalEmbed.setColor(0xe74c3c)
                .setTitle('≡ƒÅ│∩╕Å BATTLE FORFEITED')
                .setDescription(`You abandoned the fight! **${boss.name}** claims victory by default.\n\nΓ¥î **Lost:** **${bet.toLocaleString()} Baubles**\n≡ƒöÑ **Streak:** Reset to 0 (Best: \`${baubleData.animebattleMaxStreak || 0}\`)`);
        } 
            ...
            if (bet > 0) {
                ...
                payoutInfo = `\n\n≡ƒÆ░ **Wager Winnings:** \`+${winnings.toLocaleString()} Baubles\`\n≡ƒæ¢ **New Balance:** \`${baubleData.baubles.toLocaleString()} Baubles\``;
            } else {
                payoutInfo = `\n\n≡ƒÄ« *Played for fun without wagering baubles.*`;
            }

            await baubleData.save();

            finalEmbed.setColor(0x2ecc71)
                .setTitle('≡ƒÅå VICTORY FOR THE HERO!')
                .setDescription(`≡ƒææ **${player.name}** defeated **${boss.name}** in a legendary showdown!\n\n` +
                    `≡ƒÆû **Your HP:** \`${player.hp}/${player.maxHp}\` | ≡ƒÆÇ **Boss HP:** \`0/${boss.maxHp}\`` +
                    `${payoutInfo}\n≡ƒöÑ **Win Streak:** \`${baubleData.animebattleStreak}\` (Best: \`${baubleData.animebattleMaxStreak}\`)`)
            ...
            if (bet > 0) {
                payoutInfo = `\n\n≡ƒÆ╕ **Lost:** \`-${bet.toLocaleString()} Baubles\`\n≡ƒæ¢ **New Balance:** \`${baubleData.baubles.toLocaleString()} Baubles\``;
            } else {
                payoutInfo = `\n\n≡ƒÄ« *Played for fun without wagering baubles.*`;
            }

            finalEmbed.setColor(0xe74c3c)
                .setTitle('≡ƒÆÇ YOU WERE DEFEATED')
                .setDescription(`≡ƒÆÇ **${boss.name}** has crushed **${player.name}** in the arena!\n\n` +
                    `≡ƒÆû **Your HP:** \`0/${player.maxHp}\` | ≡ƒÆÇ **Boss HP:** \`${boss.hp}/${boss.maxHp}\`` +
                    `${payoutInfo}\n≡ƒöÑ **Win Streak:** Reset to 0 (Best: \`${baubleData.animebattleMaxStreak || 0}\`)`)
        ...
        const errMsg = 'Γ¥î An unexpected error occurred during the battle showdown.';
        if (isSlash) {
