...
module.exports = {
	...
	data: new SlashCommandBuilder()
		.setName('unlock')
		.setDescription('Unlocks a channel for a specific role.')
	...
	async execute(interaction) {
		...
		try {
			...
			try {
				...
				await logServerEvent(interaction.guild.id, 'UNLOCK', `Unlocked channel #${targetChannel.name} for ${targetRole.name}. Reason: ${reason}`, interaction.user, targetChannel);
			} catch (e) {
			...
			const embed = new EmbedBuilder()
				...
				.setDescription(`The channel ${targetChannel} has been unlocked for **${targetRole.name}**.`)
				.setColor(0x00cc66)
				.addFields(
			...
			console.error(error);
			await interaction.reply({ content: `❌ Failed to unlock channel ${targetChannel}. Make sure I have "Manage Channels" permission.`, ephemeral: true });
		}
	},
	...
	async executePrefix(message, args) {
		...
		try {
			...
			try {
				const { logServerEvent } = require('../../utils/serverLogger');
				await logServerEvent(message.guild.id, 'UNLOCK', `Unlocked channel #${targetChannel.name} for ${targetRole.name}. Reason: ${reason}`, message.author, targetChannel);
			} catch (e) {
				console.error('[unlock] Logging failed:', e);
			}
			...
			console.error(error);
			await message.reply(`❌ Failed to unlock channel ${targetChannel}. Make sure I have "Manage Channels" permission.`);
		}
	}
};
