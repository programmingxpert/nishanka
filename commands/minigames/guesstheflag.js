...
async function runFlagGame(initialMessageOrInteraction, channel) {
    ...
    try {
        ...
        for (let round = 1; round <= totalRounds; round++) {
            ...
            client.activeGuesstheflagGames.set(channel.id, {
        ...
        if (scores.size === 0) {
            await channel.send({ content: '🏁 The game has ended! Nobody scored any points.' });
            return;
        }
        ...
        if (client && client.activeGuesstheflagGames) {
            client.activeGuesstheflagGames.delete(channel.id);
        }
    }
}
