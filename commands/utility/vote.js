...
function computeRewards(voteData) {
    const streak  = voteData.voteStreak + 1; // will be this after increment
...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('vote')
        .setDescription('Vote for Nishanka on Top.gg and see your vote rewards!'),
    ...
    async execute(interaction) {
        ...
        const embed = new EmbedBuilder()
            ...
            .setThumbnail(`https://cdn.discordapp.com/avatars/${interaction.client.user.id}/${interaction.client.user.avatar}.png`)
            .addFields(
                {
                ...
                {
                    name: canVote ? '🎁 Your Next Vote Reward' : '🎁 Next Vote Reward (Preview)',
                    value: [
            ...
            .setFooter({ text: 'Voting takes 5 seconds and helps others discover Nishanka ✨' });

        if (!canVote && nextVote) {
            embed.setDescription(`You've already voted! Come back <t:${nextVote}:R> to keep your streak alive.`);
        } else {
            embed.setDescription(`Voting is **free**, takes **5 seconds**, and directly helps Nishanka grow.\nEvery vote means a lot — genuinely. 💙`);
        }
