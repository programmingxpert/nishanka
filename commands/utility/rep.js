...
async function giveReputation(context, guildId, sender, target, isPrefix = false) {
    ...
    try {
        ...
        // Give a few baubles to the target user (between 10 and 30 baubles)
        let targetBaubles = await Bauble.findOne({ userId: target.id });
        if (!targetBaubles) {
            targetBaubles = new Bauble({ userId: target.id, baubles: 0 });
        }
        const baubleReward = Math.floor(Math.random() * 21) + 10; // 10 to 30 baubles
        targetBaubles.baubles = (targetBaubles.baubles || 0) + baubleReward;
        await targetBaubles.save();
